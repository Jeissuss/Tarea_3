...
//FALTA //Aqui ji hay problema.

void MostrarJuegoDelAnio(TreeMap* MapValoracion,char* Fecha){
  
Pair* aux = firstTreeMap(MapValoracion);
  getchar();
...
while(aux != NULL){
  ...
  Juegos* tmpJuego = firstList(lista);
  while(tmpJuego != NULL){
    if(strcmp(tmpJuego->anio,Fecha)==0){
      printf("Nombre: %s Valoracion: %d Año: %s \n",tmpJuego->nombre,tmpJuego->valoracion,tmpJuego->anio);
      }
    tmpJuego = nextList(lista);
    }
  aux = nextTreeMap(MapValoracion);
